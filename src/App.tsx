...
export default function App() {
  ...
  const handleResolveAlert = (alertId: string, recommendation: string) => {
    ...
    } else if (alert.type === 'proactive_replenishment') {
      // Proactive Replenishment: calculate restock quantity
      const currentInv = inventory.find(i => i.productId === alert.productId && i.storeId === alert.storeId);
      const restockAmt = currentInv ? Math.round(currentInv.reorderPoint * 1.5) : 30;
      handleUpdateStock(alert.productId, alert.storeId, restockAmt);
  ...
  // Sustained 3-day negative inventory trends even when stock is above reorder points (Proactive Replenishment Warning)
  const proactiveAlerts = React.useMemo(() => {
    const list: OperationalAlert[] = [];
    
    // Sort and get the last 3 days dynamically from sales history to be resilient
    const dates = Array.from(new Set(salesHistory.map(r => r.date))).sort();
    if (dates.length < 3) return list;
    const d1 = dates[dates.length - 1]; // Day 1 (latest)
    const d2 = dates[dates.length - 2]; // Day 2
    const d3 = dates[dates.length - 3]; // Day 3
    
    inventory.forEach(item => {
      // 1. Must be above reorder point
      if (item.stock <= item.reorderPoint) return;
      
      // 2. Check for continuous sales on all 3 of the last days (sustained negative inventory trend / daily depletion)
      const r1 = salesHistory.find(r => r.productId === item.productId && r.storeId === item.storeId && r.date === d1)?.unitsSold || 0;
      const r2 = salesHistory.find(r => r.productId === item.productId && r.storeId === item.storeId && r.date === d2)?.unitsSold || 0;
      const r3 = salesHistory.find(r => r.productId === item.productId && r.storeId === item.storeId && r.date === d3)?.unitsSold || 0;
      
      if (r1 > 0 && r2 > 0 && r3 > 0) {
        const totalSales3Day = r1 + r2 + r3;
        const avgVelocity3Day = totalSales3Day / 3;
        
        // Days remaining until stock reaches the reorder point
        const safetyUnits = item.stock - item.reorderPoint;
        const daysToReorderPoint = safetyUnits / avgVelocity3Day;
        
        // Warning threshold: will hit reorder point in 15 days or less at current velocity
        if (daysToReorderPoint <= 15) {
          const prod = products.find(p => p.id === item.productId);
          const store = stores.find(s => s.id === item.storeId);
          if (prod && store) {
            const id = `proactive-${item.storeId}-${item.productId}`;
            
            // Check if manually resolved
            if (resolvedAlertIds.includes(id)) return;
            
            list.push({
              id,
              type: 'proactive_replenishment',
              productId: item.productId,
              storeId: item.storeId,
              title: `Proactive Replenishment: ${prod.name}`,
              description: `Sustained 3-day negative inventory trend detected at ${store.name}. Stock is above the reorder point of ${item.reorderPoint} units, but consistent daily demand will deplete this safety buffer in ${daysToReorderPoint.toFixed(1)} days at a velocity of ${avgVelocity3Day.toFixed(1)} units/day.`,
              metric: `${daysToReorderPoint.toFixed(1)} Days to Reorder`,
              calculation: `Stock: ${item.stock} - Reorder Point: ${item.reorderPoint} = ${safetyUnits} safety units remaining above safety line. Avg velocity of last 3 days = (${r1} + ${r2} + ${r3}) / 3 = ${avgVelocity3Day.toFixed(2)} units/day. ${safetyUnits} / ${avgVelocity3Day.toFixed(2)} = ${daysToReorderPoint.toFixed(1)} days to reach reorder point.`,
              recommendation: `Place a proactive replenishment order of ${Math.round(item.reorderPoint * 1.5)} units immediately to avoid critical safety stock depletion and maintain customer shelf availability.`,
              actionLabel: `Proactive Restock +${Math.round(item.reorderPoint * 1.5)} Units`,
            });
          }
        }
      }
    ...
    return list;
  }, [inventory, salesHistory, products, stores, resolvedAlertIds]);

  const allAlerts = React.useMemo(() => {
    return [...alerts, ...proactiveAlerts].filter(a => !resolvedAlertIds.includes(a.id));
  }, [alerts, proactiveAlerts, resolvedAlertIds]);
  ...
  const summaryMetrics = React.useMemo(() => {
    ...
    const anomalies = allAlerts.filter(a => a.type === 'sales_spike' || a.type === 'sales_drop' || a.type === 'proactive_replenishment').length;
  ...
  }, [salesHistory, products, inventory, allAlerts]);
