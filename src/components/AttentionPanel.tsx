...
export default function AttentionPanel({ 
  ...
  const getAlertIcon = (type: string) => {
    switch (type) {
      ...
      case 'proactive_replenishment':
        return <TrendingUp className="text-indigo-500 dark:text-indigo-400 w-5 h-5" />;
      default:
  ...
  const getAlertTheme = (type: string) => {
    switch (type) {
      ...
      case 'proactive_replenishment':
        return {
          bg: 'bg-indigo-50/70 dark:bg-indigo-950/10 hover:bg-indigo-100/50 dark:hover:bg-indigo-950/20',
          border: 'border-indigo-200 dark:border-indigo-900/40',
          text: 'text-indigo-800 dark:text-indigo-300',
          badge: 'bg-indigo-100 text-indigo-800 border-indigo-200 dark:bg-indigo-950/50 dark:text-indigo-300 dark:border-indigo-900/50'
        };
      case 'slow_moving':
        return {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-6" id="attention-panel">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5 border-b border-gray-100 dark:border-slate-800 pb-4">
        <div>
        ...
        <div className="flex items-center gap-2.5">
          {alerts.length > 0 && (
            <button
              ...
              className="inline-flex items-center justify-center px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-semibold text-xs rounded-lg shadow-3xs cursor-pointer transition-all"
            >
              {isResolvingAll ? (
